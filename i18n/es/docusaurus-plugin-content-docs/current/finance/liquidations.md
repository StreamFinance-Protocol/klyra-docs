...
- BA: ajuste de quiebra en partes por millón (este es un valor constante establecido en la configuración de la cadena que puede cambiarse mediante una votación de gobernanza en la blockchain)

- SMMR: ratio de margen de mantenimiento (este es un valor constante establecido en la configuración de la cadena que puede cambiarse mediante una votación de gobernanza en la blockchain)
...
Como se mencionó anteriormente, las liquidaciones se activan cuando `TNC = TMMR`, por lo que en ese punto el precio llenable será el precio del oráculo - es decir, intentamos cerrar las posiciones no saludables al precio actual (sin descuento). Si el precio continúa moviéndose en contra del trader, y su TNC continúa bajando, `TNC / TMMR` se vuelve menor que 1, lo que significa que comenzamos a intentar vender la posición con descuento, proporcionando efectivamente un incentivo adicional para que alguien compre la posición.
